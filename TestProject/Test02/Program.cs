...
namespace Test02 {
    ...
    class Program {
        ...
        private static void Exercise03(List<int> numbers) {
            var nums = numbers.Where(x => x % 2 == 1).OrderByDescending(x => x);
        ...
        private static void Exercise04(List<int> numbers) {
            var nums = numbers.Where(x => x >= 10 && x <= 50);
            foreach (var x in nums) Console.Write(x + " ");
        }
        ...
        private static void Exercise05(List<Person> persons) {
            Console.WriteLine(persons.Average(x => x.Age)+"才");
        }

        //問題６　身長170cm以上の体重平均を表示
        //　　　　出力結果【72.25kg】
        private static void Exercise06(List<Person> persons) {
            Console.WriteLine(persons.Where(x => x.Height >= 170).Average(x => x.Weight)+"kg");
        }
        ...
        private static void Exercise07(List<Person> persons) {
            var person = persons.Where(x => x.Weight <= 70);
            foreach(var x in person) Console.WriteLine(x.Name + " " + x.Weight + "kg");
        }
        ...
        private static void Exercise08(List<Person> persons) {
            var person = persons.Where(x =>x.Name.Contains("山"));
            foreach (var x in person) Console.WriteLine(x.Name);
        }
    }
}
